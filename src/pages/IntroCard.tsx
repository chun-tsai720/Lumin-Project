    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'opacity 0.5s' }} className={isExiting ? 'opacity-0' : 'opacity-100'}>
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: '80px', maxWidth: '1000px', width: '100%', padding: '0 40px' }}>
        ...
        <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
          {/* 用最高權重 style 鎖死正方形 180px，絕對不可能再變大 */}
        ...
        <section style={{ maxWidth: '420px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <nav style={{ display: 'flex', gap: '16px' }}>
            <button
              style={{ padding: '10px 20px', fontSize: '11px', letterSpacing: '0.25em', color: '#71717a', border: '1px solid #27272a', backgroundColor: 'transparent', cursor: 'pointer' }}
              className="hover:border-[#DDAA33] hover:text-[#DDAA33] transition-all duration-300"
              onClick={() => handleNavigation('/')}
            >
              ← BACK TO COVER
            </button>
            <button
              style={{ padding: '10px 20px', fontSize: '11px', letterSpacing: '0.25em', color: '#DDAA33', border: '1px solid #27272a', backgroundColor: 'transparent', cursor: 'pointer' }}
              className="hover:border-[#DDAA33] hover:brightness-125 transition-all duration-300"
